...
import {getClientModel, cnnColors} from './taskDetailConstants'

const {t} = useI18n()

const props = defineProps<{
  clientMetrics: { accuracy: number }[]
}>()

const selectedClientMetric = ref<'accuracy'>('accuracy')
...
const clientDetailChartData = computed(() => {
  ...
  const acc = props.clientMetrics[clientIdx]?.accuracy ?? 0.5
  const init = acc * 0.18
  const points: number[] = []
  for (let r = 0; r < numRounds.value; r++) {
    ...
    points.push(Math.max(0.01, Math.min(0.99, init + (acc - init) * progress + noise)))
  }
  points[points.length - 1] = acc
  return points
})
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (params: { axisValue?: number; data?: number }[]) => {
        ...
        const val = typeof params[0]?.data === 'number' ? (params[0].data * 100).toFixed(2) + '%' : '-'
    ...
    series: [{
      name: t('pages.recommended.clientMetricAccuracy'),
      type: 'line',
      data,
      smooth: 0.25,
      symbol: 'none',
...
watch([selectedClientId, clientDetailChartData], () => {
  ...
  <section class="task-detail-section task-detail-tech-card relative overflow-hidden rounded-xl py-5 px-6 shrink-0">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 relative z-[1]">
      ...
      >
        ...
        <div class="task-detail-client-ring-wrap">
          <svg viewBox="0 0 36 36" class="task-detail-client-svg">
            ...
            <circle
                ...
                :stroke-dashoffset="100 - (clientMetrics[i - 1]?.accuracy ?? 0) * 100"
            />
          </svg>
          <span class="task-detail-client-value tabular-nums">
            {{ ((clientMetrics[i - 1]?.accuracy ?? 0) * 100).toFixed(1) }}%
          </span>
        </div>
        <button
            type="button"
...
.task-detail-client-svg {
  width: 100%;
  height: 100%;
  transform: rotate(-90deg);
}

.task-detail-client-value {
  position: absolute;
  inset: 0;
  display: flex;
  align-items: center;
