...
public class TaskController extends BaseController {
    ...
    /**
     * 创建任务。同配置且已有 SUCCESS/RECOMMENDED 任务时复制结果（返回 copied=true）；否则新建 NOT_STARTED（返回 copied=false）。
     *
     ...
     * @return 操作结果
     */
    @Operation(summary = "删除任务接口", description = "逻辑删除任务，将 is_deleted 置为 1；推荐和正在进行中的任务不可删；本人或管理员可删")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "成功"),
