...
- 🔐 认证与权限：注册登录找回、JWT、验证码、限流与黑名单。
- 🧭 任务生命周期：创建查询、启动停止、删除推荐与访问隔离。
- 🔄 实时链路：Python → Redis → Spring Boot → MySQL/WebSocket。
- 📈 可视化：指标卡片、轮次曲线、客户端网格与客户端历史。
- 🖥️ 仪表盘：平台统计、趋势分析、健康检查与实时资源监控。
- 🎨 体验能力：深浅色主题与中英文多语言支持。
- 🤖 智能助手：基于 RAG 的工作流智能体，支持会话与流式问答。
